/**********************************************************************
 *
 * qsort-main-omp.c -- Sequential and Parallel implementation of QuickSort
 *
...
double qsort_time ,seq_time, par_time,mean_qsort ,mean_seq ,mean_par;
  int n_it,n,n1,n2,thr_it,thr1,thr2,k,nThreads,meanSize, pass;
    ...
    for(thr_it = thr1;thr_it < thr2+1;thr_it++){
      ...
      for(k=0;k<meanSize;k++){
        ...
        */
        ...
        #pragma omp parallel// shared(a_par)
        {
          #pragma omp single
          {
           qsort_omp(a_par, n);
  ...
  if(argc == 5){
    fclose(f);
    printf("File omp_data.csv  was written successfully\n");
  }
  /* exit */
  return 0;
